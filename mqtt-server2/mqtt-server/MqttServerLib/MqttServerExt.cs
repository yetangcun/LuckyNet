...
namespace MqttServerLib
{
    ...
    /// MqttServer服务端实现
    /// </summary>
    public class MqttServerExt : IMqttServerExt
    {
        // private MqttServer? _mqttServer;
        // private readonly MqttServerOption _option;
         //private readonly ILogger<MqttServerExt> _logger;
        ...
        /// 静态构造函数
        /// </summary>
        static MqttServerExt()
        {
            _cltDic = new ConcurrentDictionary<string, ClientInfo>();
            _cltSubscribes = new ConcurrentDictionary<string, List<string>>();  // 客户端订阅的主题列表
        }
        ...
        private async Task _Init()
        {
            try
            {
                Log.Information($"MqttServer【::{_option.Port}】正在初始化...");
                if (_mqttServer != null)
                {
            ...
            {
                Log.Error($"MQTT服务启动失败:{ex.Message}");
            }
        }
        ...
        public async Task StartAsync()
        {
            try
            {
                if (_mqttServer != null && _mqttServer.IsStarted)
                {
                    Log.Information($"MqttServer【::{_option.Port}】正在运行");
                    return;
                }
                ...
                Log.Information($"MqttServer【::{_option.Port}】正在启动...");
                await _mqttServer!.StartAsync();
            }
            catch (Exception ex)
            {
                Log.Error($"MqttServer【::{_option.Port}】启动失败:{ex.Message}");
            }
        }
        ...
        private static ConcurrentDictionary<string, ClientInfo> _cltDic; // 客户端信息
        private static ConcurrentDictionary<string, List<string>> _cltSubscribes;  // 客户端订阅的主题列表
        ...
        /// <returns></returns>
        public static Task OnStartedAsync(EventArgs arg)
        {
            Log.Information($"MqttServer【::{_option.Port}】启动成功!");
            return Task.CompletedTask;
        }
        ...
        /// <returns></returns>
        public static Task OnStoppedAsync(EventArgs args)
        {
            try
            {
                Log.Information($"MqttServer【::{_option.Port}】已停止!");
            }
            catch (Exception ex)
            {
                Log.Error($"MqttServer【::{_option.Port}】停止异常：{ex.Message},{ex.StackTrace},{ex.InnerException}");
            }
            return Task.CompletedTask;
        }
        ...
        public static Task OnValidatingConnection(ValidatingConnectionEventArgs cxt)
        {
            ...
            else
            {
                ...
                Log.Error($"客户端{cxt.ClientId}请求连接失败:{cxt.ReasonCode},{cxt.ReasonString}");
            }
        ...
        /// <param name="args"></param>
        /// <returns></returns>
        public static Task OnClientConnected(ClientConnectedEventArgs args)
        {
            ...
            if (_cltDic.ContainsKey(args.ClientId))
            {
                ...
                Log.Information($"客户端{args.ClientId}接入, 在线客户端总数:{_cltDic.Count()}");
                return Task.CompletedTask;
            }
        ...
        /// <param name="args"></param>
        /// <returns></returns>
        public static Task OnClientDisconnected(ClientDisconnectedEventArgs args)
        {
            Log.Information($"客户端{args.ClientId}已断开");
        ...
        /// <param name="args"></param>
        /// <returns></returns>
        public static Task OnClientSubscribedTopic(ClientSubscribedTopicEventArgs args)
        {
            Log.Information($"客户端{args.ClientId}订阅了主题{args.TopicFilter.Topic}\r\n");
        ...
        /// <param name="args"></param>
        /// <returns></returns>
        public static Task OnClientUnsubscribedTopic(ClientUnsubscribedTopicEventArgs args)
        {
            Log.Information($"客户端{args.ClientId}取消订阅主题{args.TopicFilter}\r\n");
        ...
        {
            Log.Warning($"未处理消息:{args.ApplicationMessage.Topic},{UTF8Encoding.UTF8.GetString(args.ApplicationMessage.Payload.ToArray())}");
            return Task.CompletedTask;
        }

        private Task OnStartedAsync1(EventArgs arg)
        {
            Log.Information($"MqttServer【::{_option.Port}】启动成功!");
            return Task.CompletedTask;
        }

        private async Task OnStoppedAsync1(EventArgs args)
        {
            try
            {
                Log.Information($"MqttServer【::{_option.Port}】已停止!");
                ...
                {
                    await _Init();          // 重新初始化
                    await StartAsync();     // 调用StartAsync方法重启
                }
            }
            catch (Exception ex)
            {
                Log.Error($"MqttServer【::{_option.Port}】 重启失败:{ex.Message}--{ex.StackTrace}--{ex.InnerException}!");
            }
        }

        private Task OnApplicationMessageNotConsumed1(ApplicationMessageNotConsumedEventArgs args)
        {
            Log.Warning($"未处理消息:{args.ApplicationMessage.Topic},{UTF8Encoding.UTF8.GetString(args.ApplicationMessage.Payload.ToArray())}");
            return Task.CompletedTask;
        }

        private Task OnValidatingConnection1(ValidatingConnectionEventArgs cxt)
        {
            if (cxt.UserName == _option.UserName && cxt.Password == _option.PassWord)
            {
                cxt.ReasonCode = MqttConnectReasonCode.Success;
        ...
        private Task OnClientConnected1(ClientConnectedEventArgs args)
        {
            var clitInfo = new ClientInfo
            {
                ClientId = args.ClientId,
        ...
        private Task OnClientDisconnected1(ClientDisconnectedEventArgs args)
        {
            Log.Information($"客户端{args.ClientId}已断开");
            _cltDic.TryRemove(args.ClientId, out _);         // 删除客户端信息
            _cltSubscribes.TryRemove(args.ClientId, out _);  // 删除客户端订阅的Topic
        ...
        private Task OnClientSubscribedTopic1(ClientSubscribedTopicEventArgs args)
        {
            Log.Information($"客户端{args.ClientId}订阅了主题{args.TopicFilter.Topic}\r\n");
        ...
        private Task OnClientUnsubscribedTopic1(ClientUnsubscribedTopicEventArgs args)
        {
            Log.Information($"客户端{args.ClientId}取消订阅主题{args.TopicFilter}\r\n");

...
namespace Data.EFCore.Cxt
{
    ...
    public class CommonCxt : DbContext, ICommonCxt
    {
        ...
        public TCxt GetDbCxt<TCxt, TOpt>(TOpt opt) where TCxt : CommonCxt where TOpt : EfcoreOption
        {
            _opt = opt;
            return (TCxt)this;
        }
        ...
        {   
            ...
            var connString = _opt.IsReadOnly ? _opt.SlaveConnectionString : _opt.MasterConnectionString;
            switch (_opt.DatabaseType)
            {
                case DatabaseType.Mysql:
                    //optsBlder.UseMySql(connString, ServerVersion.AutoDetect(connString), blder =>  // 使用Pomelo. EntityFrameworkCore.MySql 时需添加此配置
                    //{
                    //    blder.CommandTimeout(_opt.DefaultTimeOut);
                    //});
